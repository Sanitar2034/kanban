...
export default function App(): ReactElement {
	...
	const handleBackWithCodeBrowserReset = useCallback(() => {
		handleBack();
		setIsGitHistoryOpen(false);
	...
	const handleToggleGitHistory = useCallback(() => {
		...
		setIsGitHistoryOpen((current) => {
			if (!current) setIsCodeBrowserOpen(false);
			return !current;
		});
	}, [hasNoProjects]);
	...
	}, [hasNoProjects, setSelectedTaskId]);
			...
			<div className="flex h-[100svh] min-w-0 overflow-hidden">
				...
				<div className="flex flex-col flex-1 min-w-0 overflow-hidden">
					<TopBar
						...
						onBack={selectedCard ? handleBackWithCodeBrowserReset : undefined}
						...
						onToggleCodeBrowser={hasNoProjects ? undefined : handleToggleCodeBrowser}
								...
								<div className="flex flex-1 flex-col min-h-0 min-w-0">
									<div className="flex flex-1 min-h-0 min-w-0">
										{isCodeBrowserOpen ? (
											<CodeBrowserPanel
												workspaceId={currentProjectId}
												externalFilePath={codeBrowserPendingFilePath}
											/>
										) : isGitHistoryOpen ? (
											<GitHistoryView
												workspaceId={currentProjectId}
