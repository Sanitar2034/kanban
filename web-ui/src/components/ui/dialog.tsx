...
export function DialogHeader({
	...
	return (
		<div className="flex items-center justify-between px-2 py-2 bg-surface-2 border-b border-[#5A6572] shrink-0 rounded-t-lg max-md:px-3 max-md:py-3">
			...
			<RadixDialog.Close className="p-1 rounded-md text-text-tertiary hover:text-text-primary hover:bg-surface-3 cursor-pointer max-md:min-h-11 max-md:min-w-11 max-md:flex max-md:items-center max-md:justify-center">
				<X size={16} className="max-md:!size-5" />
			</RadixDialog.Close>
		</div>
	);
}
...
export function DialogBody({ children, className }: { children: ReactNode; className?: string }): React.ReactElement {
	return (
		<div className={cn("p-4 overflow-y-auto flex-1 min-h-0 bg-surface-1 overscroll-contain", className)}>
			{children}
		</div>
	);
}
...
export function DialogFooter({ children }: { children: ReactNode }): React.ReactElement {
	return (
		<div className="flex justify-end gap-2 px-2 py-2 bg-surface-2 border-t border-[#5A6572] shrink-0 rounded-b-lg max-md:px-3 max-md:py-3 max-md:gap-3">
			{children}
		</div>
	);
}
...
export function AlertDialogHeader({
	...
	return (
		<div
			className={cn(
				"px-2 py-2 bg-surface-2 border-b border-[#5A6572] shrink-0 rounded-t-lg max-md:px-3 max-md:py-3",
				className,
			)}
		>
			{children}
...
export function AlertDialogFooter({
	...
	return (
		<div
			className={cn(
				"flex justify-end gap-2 px-2 py-2 bg-surface-2 border-t border-[#5A6572] shrink-0 rounded-b-lg max-md:px-3 max-md:py-3 max-md:gap-3",
				className,
			)}
		>
			{children}
