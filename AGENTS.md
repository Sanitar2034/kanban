...
- Detail diff render cost scales with expanded file count; defaulting all files expanded in `DiffViewerPanel` can stall task open on large worktrees. Keep only the selected file expanded by default unless explicitly optimizing for full-document scan views.
- On restore-from-trash for terminal agents (including Codex), preserve and seed the prior terminal snapshot into the new `TerminalStateMirror` before relaunching. If we dispose the old mirror first, the detail view appears blank until fresh agent output arrives, which looks like "slow hydration" even when resume started successfully.
- If Kanban is launched via `npm run ...`, npm prepends `node_modules/.bin` to PATH. This repo's transitive `@openai/codex` dependency can expose an old `node_modules/.bin/codex` (e.g. 0.105.0), so task sessions may launch stale Codex and show blocking startup update prompts. Prefer a non-`node_modules/.bin` Codex executable for runtime sessions and disable startup update checks in managed Kanban Codex launches.
