...
- **Desktop DMG builds can silently ship stale code.** The desktop package depends on `kanban` via `"file:kanban-X.Y.Z.tgz"` in `package.json`. If you bump the root kanban version, rebuild, and `npm pack`, you MUST also update the desktop `package.json` to reference the new tarball filename — otherwise `npm install` happily uses the cached old version and the DMG ships stale runtime code. The symptom is that `/api/health` returns the old version number. Use `node packages/desktop/scripts/fresh-build.mjs` for guaranteed-correct builds; it handles the nuclear clean, tarball packing, version update, and DMG build in one shot.
- **Don't use `fetch` or `net.fetch` for HTTP requests from the Electron main process in packaged apps.** Both Electron's `net.fetch` and the global `fetch` silently fail for localhost HTTP connections in packaged (signed/notarized) desktop builds — `curl` works fine from the terminal, but the same request returns errors inside Electron. Use Node.js native `http.get`/`https.get` instead, which always works because it bypasses Chromium's network stack entirely. The desktop `ConnectionManager.checkRemoteHealth()` hit this; the fix was switching from `fetch` to `http.get`.

- **Desktop must not overwrite the CLI's runtime descriptor.** When the desktop app boots and detects a `terminal-owned` runtime descriptor (from a CLI `kanban` process), it starts its own child on a separate port — but it must NOT call `publishRuntimeDescriptor()` or `clearRuntimeDescriptor()`. Overwriting the descriptor steals it from the CLI, and clearing it on desktop quit causes all agent tasks running against the CLI server to lose their runtime discovery endpoint (manifests as cascading `TimeoutError` and `session_stop` errors in `postToApi`). The `terminalOwnsDescriptor` flag in `main.ts` gates this.
- **Desktop window persistence and macOS hide-on-close.** On macOS, only the last visible window hides on close (keeps the app in the dock); closing any other window destroys it normally. `saveAllStates()` saves all windows (including the hidden last one). Duplicate project windows are allowed — there is no deduplication on create or load. The `lastViewedPath` field in `PersistedWindowState` captures the URL pathname each window was viewing, so non-locked overview windows restore to the correct project.
