...
- Codex task runtime is no longer a PTY-per-task wrapper. Kanban now keeps one workspace-scoped `codex app-server` host inside `TerminalSessionManager`, persists each task's exact `agentSessionId` plus terminal snapshot, and restores by reattaching that thread on the warm host. If you need to change Codex restore/start behavior, start in `src/terminal/session-manager.ts`, `src/codex-sdk/codex-app-server-client.ts`, and `web-ui/src/terminal/persistent-terminal-manager.ts` instead of trying to extend `prepareAgentLaunch()` or `resume --last`.
- Codex `turn/completed` is not the same thing as the session/process exiting. The shared app-server host stays alive, and the task remains reusable. Do not surface Codex turn completion as `session exited with code 0` in the terminal UI; that message makes working restores and follow-up input look broken. Prefer an explicit "turn completed, type another instruction to continue" prompt instead.
- Codex trash/restore cannot rely only on the debounced `summary.terminalRestoreSnapshot`. `startTaskSession()` rebuilds the live mirror before the browser reconnects, and `getRestoreSnapshot()` prefers that live mirror over summary state. If you do not capture the old live `terminalStateMirror` snapshot and seed the new mirror before reattach, fast trash -> restore comes back blank even though the prior screen buffer existed in memory.
